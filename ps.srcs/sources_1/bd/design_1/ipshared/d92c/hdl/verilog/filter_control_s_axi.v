...
#(parameter
    ...
    output wire [63:0]                   op,
    output wire [63:0]                   comp1,
    output wire [63:0]                   comp2
);
//------------------------Address Info-------------------
...
localparam
    ...
    ADDR_AP_RETURN_1  = 7'h14,
    ADDR_IN_R_DATA_0  = 7'h1c,
    ADDR_IN_R_DATA_1  = 7'h20,
    ADDR_IN_R_CTRL    = 7'h24,
    ADDR_OUT_R_DATA_0 = 7'h28,
    ADDR_OUT_R_DATA_1 = 7'h2c,
    ADDR_OUT_R_CTRL   = 7'h30,
    ADDR_NUM_DATA_0   = 7'h34,
    ADDR_NUM_DATA_1   = 7'h38,
    ADDR_NUM_CTRL     = 7'h3c,
    ADDR_OP_DATA_0    = 7'h40,
    ADDR_OP_DATA_1    = 7'h44,
    ADDR_OP_CTRL      = 7'h48,
    ADDR_COMP1_DATA_0 = 7'h4c,
    ADDR_COMP1_DATA_1 = 7'h50,
    ADDR_COMP1_CTRL   = 7'h54,
    ADDR_COMP2_DATA_0 = 7'h58,
    ADDR_COMP2_DATA_1 = 7'h5c,
    ADDR_COMP2_CTRL   = 7'h60,
    WRIDLE            = 2'd0,
    WRDATA            = 2'd1,
    WRRESP            = 2'd2,
    WRRESET           = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_num = 'b0;
    reg  [63:0]                   int_op = 'b0;
    reg  [63:0]                   int_comp1 = 'b0;
    reg  [63:0]                   int_comp2 = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_COMP1_DATA_0: begin
                    rdata <= int_comp1[31:0];
                end
                ADDR_COMP1_DATA_1: begin
                    rdata <= int_comp1[63:32];
                end
                ADDR_COMP2_DATA_0: begin
                    rdata <= int_comp2[31:0];
                end
                ADDR_COMP2_DATA_1: begin
                    rdata <= int_comp2[63:32];
                end
            endcase
        end
    end
...
assign num       = int_num;
assign op        = int_op;
assign comp1     = int_comp1;
assign comp2     = int_comp2;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_NUM_DATA_1)
            int_num[63:32] <= (WDATA[31:0] & wmask) | (int_num[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COMP1_DATA_0)
            int_comp1[31:0] <= (WDATA[31:0] & wmask) | (int_comp1[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COMP1_DATA_1)
            int_comp1[63:32] <= (WDATA[31:0] & wmask) | (int_comp1[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COMP2_DATA_0)
            int_comp2[31:0] <= (WDATA[31:0] & wmask) | (int_comp2[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COMP2_DATA_1)
            int_comp2[63:32] <= (WDATA[31:0] & wmask) | (int_comp2[63:32] & ~wmask);
    end
end
